...
export const CreateOrderCouponSchema = z
  .object({
    name: z
      .string()
      .min(1, '쿠폰명을 입력해주세요.')
    ...
    type: z.enum(CouponType, {
      message: '쿠폰 타입은 RATE 또는 AMOUNT여야 합니다.',
    }),

    discountAmount: z
      .number()
      .int('할인 금액은 정수여야 합니다.')
      .positive('할인 금액은 양수여야 합니다.')
      .optional()
      .nullable(),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.type === 'AMOUNT') {
      if (!data.discountAmount) {
        ctx.addIssue({
          code: 'custom',
          message: 'AMOUNT 타입 쿠폰은 할인 금액이 필수입니다.',
          path: ['discountAmount'],
        });
      }
      ...
      if (!data.discountRate) {
        ctx.addIssue({
          code: 'custom',
          message: 'RATE 타입 쿠폰은 할인율이 필수입니다.',
          path: ['discountRate'],
        });
      }
    }
...
export class CreateOrderCouponDto extends createZodDto(
  CreateOrderCouponSchema,
) {}
